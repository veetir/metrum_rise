...
fn vegetation_removal_matches_scatter_disc_in_both_layers() {
    ...
    let count = remove_at(&mut core, pos, radius);
    assert!(count > 1);
    assert_eq!(
        count * 6,
    ...
    assert_eq!(remove_at(&mut core, pos, radius), 0);
}
...
fn vegetation_paint_fills_rejected_candidates_and_leaves_generated_cells_alone() {
    ...
    assert_eq!(
        paint_at(&mut core, Vector2::new(generated.x, generated.z), 0.01, 1),
        0
    );
    ...
    assert_eq!(
        paint_at(&mut core, Vector2::new(rejected.x, rejected.z), 0.01, 1),
        1
    );
...
fn vegetation_repaint_restores_generated_tree_and_prunes_tombstone() {
    ...
    assert_eq!(remove_at(&mut core, pos, 0.01), 1);
    assert_eq!(core.vegetation_edits.len(), 1);
    ...
    assert_eq!(paint_at(&mut core, pos, 0.01, i64::from(p.species)), 1);
    assert_eq!(core.vegetation_edits.len(), 0);
...
fn vegetation_repaint_with_another_species_does_not_regrow_the_cleared_plant() {
    ...
    assert_eq!(paint_at(&mut core, pos, 0.01, i64::from(painted)), 1);
    ...
    assert_eq!(paint_at(&mut core, pos, 0.01, i64::from(painted)), 0);
}
...
fn vegetation_patch_revisions_touch_only_changed_plants() {
    ...
    assert_eq!(remove_at(&mut core, Vector2::ONE, 0.01), 1);
    ...
    assert_eq!(remove_at(&mut core, Vector2::ZERO, 2.0), 2);
...
fn vegetation_invalid_inputs_store_nothing_and_empty_removal_is_sparse() {
    ...
    for species in [-1, 4] {
        assert!(!add_at(&mut core, Vector2::ZERO, species));
        assert_eq!(paint_at(&mut core, Vector2::ZERO, 8.0, species), 0);
    }
    for radius in [-1.0, f32::NAN, f32::INFINITY, 1025.0] {
        assert_eq!(remove_at(&mut core, Vector2::ZERO, radius), 0);
        assert_eq!(paint_at(&mut core, Vector2::ZERO, radius, 0), 0);
    }
    let p = first_candidate(&core, false);
    assert_eq!(remove_at(&mut core, Vector2::new(p.x, p.z), 0.0), 0);
    assert_eq!(core.vegetation_edits.len(), 0);
    ...
    core.watermap
        ...
        .unwrap();
    assert!(!add_at(&mut core, Vector2::ZERO, 0));
    assert_eq!(paint_at(&mut core, Vector2::ZERO, 32.0, 0), 0);
    assert_eq!(core.vegetation_edits.len(), 0);
}
...
fn vegetation_save_round_trip_reproduces_scatter_and_v60_loads_empty_delta() {
    ...
    core.create_blank_world_internal(2560.0, 2560.0, 10.0, 640.0, 20.0)
        .unwrap();
    let p = first_candidate(&core, true);
    assert_eq!(remove_at(&mut core, Vector2::new(p.x, p.z), 0.01), 1);
    assert!(add_at(&mut core, Vector2::new(3.25, -32.0), 3));
    assert!(paint_at(&mut core, Vector2::ZERO, 45.0, 1) > 0);
    let before = [scatter(&core, true), scatter(&core, false)];
    let path =
...
fn vegetation_edit_benchmark() {
    ...
    for background in [0, 100_000] {
        ...
        remove_at(&mut core, center, 64.0);
        let expected_added = paint_at(&mut core, center, 64.0, 0);
        group.bench_function(format!("paint_{background}"), |b| {
            b.iter_custom(|iterations| {
                ...
                for _ in 0..iterations {
                    ...
                    let added = black_box(paint_at(&mut core, center, 64.0, 0));
        ...
        let painted = paint_at(&mut core, center, 255.0, 0);
        assert!(
            painted > 100,
...
fn vegetation_brush_benchmark() {
    ...
    remove_at(&mut core, Vector2::ZERO, 64.0);
    let expected = paint_at(&mut core, Vector2::ZERO, 64.0, 0);
    eprintln!(
    ...
    criterion.bench_function("VegetationBrush/paint_64m", |b| {
        b.iter_custom(|iterations| {
            ...
            for _ in 0..iterations {
                remove_at(&mut core, Vector2::ZERO, 64.0);
                let start = std::time::Instant::now();
                let added = paint_at(&mut core, Vector2::ZERO, 64.0, 0);
                elapsed += start.elapsed();
...
fn vegetation_brush_fills_fine_grid_in_occupied_canopy_cells_and_is_idempotent() {
    let mut core = core();
    let before = scatter(&core, true);
    let count = paint_at(&mut core, Vector2::ZERO, 64.0, 3);
    assert!(
    ...
    assert_eq!(paint_at(&mut core, Vector2::ZERO, 64.0, 3), 0);
...
fn vegetation_brush_spacing_is_independent_and_parallel_order_is_stable() {
    ...
    for workers in [1, 4] {
        let pool = rayon::ThreadPoolBuilder::new()
            ...
            .unwrap();
        let mut core = core();
        pool.install(|| paint_at(&mut core, Vector2::new(-20.0, 15.0), 64.0, 2));
    ...
    for canopy_m in [8.0, 16.0, 32.0] {
        ...
        paint_at(&mut core, Vector2::ZERO, 16.0, 1);
        let (_, salt) = grid(&core, VegetationLayer::Canopy);
...
fn vegetation_brush_bound_and_small_jitter() {
    let mut core = core();
    assert_eq!(
        paint_at(&mut core, Vector2::ZERO, MAX_PAINT_RADIUS_M + 0.01, 0),
        0
    );
    assert_eq!(core.vegetation_edits.len(), 0);
    let fine = disc_cells(
    ...
    let added = paint_at(&mut core, Vector2::ZERO, MAX_PAINT_RADIUS_M, 0);
